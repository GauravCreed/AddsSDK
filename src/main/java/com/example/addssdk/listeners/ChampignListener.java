...
public class ChampignListener {
    ChampingInterface champingInterface;

    public void sendChampignResponse(String response, Context mContext) {
        champingInterface = (ChampingInterface) mContext;
        ...
        if (!response.matches("") && !response.contains("error")) {
            ...
            champingInterface.getChampignRespons(response);
        } else {
            champingInterface.getChampignRespons(response);
        }
